...
export const getBackgroundColorForLevel = (level) => {
  const maxLevels = 170; // Define the maximum number of levels you want to consider
  const minOpacity = 0.05; // Minimum opacity to maintain visibility
  const opacityDecrement = (1 - minOpacity) / maxLevels; // Calculate decrement per level

  // Calculate current opacity based on level
  const currentOpacity = Math.max(1 - level * opacityDecrement, minOpacity);
      ...
      if (answer) {
        ...
        setScreenColor(
          getBackgroundColorForLevel(
            userStateReference?.databaseUserDocument?.level
          )
        );
        ...
        >
          <div style={{ textShadow: "1px 1px 5px black" }}>
            {hasDiscordTag ? (
              <>
                <h4>
                  Highest level: {globalStateReference?.globalLeaderName} -
                  Level: {globalStateReference?.globalLevelCounter}
                </h4>
                <div>
                  Your level:{" "}
                  {userStateReference?.databaseUserDocument?.level || 0}
